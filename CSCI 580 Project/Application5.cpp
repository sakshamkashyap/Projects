...
//#define INFILE  "testasc.asc"
//#define OUTFILE "output.ppm"
// CHANGES:
#define INFILE  "model.asc"
#define OUTFILEPREFIX "renderings//render"
// END CHANGES

//extern int tex_fun(float u, float v, GzColor color); /* image texture function */
extern int ptex_fun(float u, float v, float alt, float slope, GzColor color); /* procedural texture function */
...
int Application5::Initialize()
{
	 ...
	 */ 
	// PROJECT CHANGE
	// NOTE: YOU HAVE TO CHANGE IN DISP.H TO ALLOW HIGHER THAN 1024 X 1024 RESOLUTION...
...
// PROJECT CHANGE:
// these are even newer, fancier locations:
camera.position[X] = -WIDTH / 2;
camera.position[Y] = 70;
camera.position[Z] = HEIGHT / 2;

camera.lookat[X] = WIDTH / 3;
camera.lookat[Y] = 0.0;
camera.lookat[Z] = HEIGHT / 2 + 1;
...
{
	// PROJECT CHANGE (the for loop)
	int			status;
	for (int iters = 1; iters <= NUMBEROFMILLENIA; iters++)
	{
		...
		*/
		...
		int leadingZeros = 5 - floor(log10(abs(iters))) + 1;
		for (int i = 0; i < leadingZeros; i++)
		{
			filename += "0";
		}
		filename += std::to_string(iters);
		filename += ".asc";
		...
		std::string outputname = OUTFILEPREFIX;
		leadingZeros = 5 - floor(log10(abs(iters))) + 1;
		for (int i = 0; i < leadingZeros; i++)
		{
			outputname += "0";
		}
		outputname += std::to_string(iters);
		outputname += ".ppm";
		...
		*/
		...
		// Only render the final one:
		if (iters == NUMBEROFMILLENIA)
		{
